...
static const t3_config_opts_t opts = {
    T3_CONFIG_VERBOSE_ERROR | T3_CONFIG_INCLUDE_DFLT, {{path, 0}}};
...
static void compare_config(t3_config_t *a, t3_config_t *b) {
  ...
  case T3_CONFIG_BOOL:
    if (t3_config_get_bool(a) != t3_config_get_bool(b))
      fatal("Different bool value at lines %d/%d: %d != %d\n",
            t3_config_get_line(a), t3_config_get_line(a), t3_config_get_bool(a),
            t3_config_get_bool(b));
    break;
  ...
  case T3_CONFIG_SECTION: {
    ...
    for (a_sub = t3_config_get(a, NULL), b_sub = t3_config_get(b, NULL);
         a_sub != NULL && b_sub != NULL;
         a_sub = t3_config_get_next(a_sub), b_sub = t3_config_get_next(b_sub))
      compare_config(a_sub, b_sub);
    if (a_sub != NULL || b_sub != NULL)
      fatal("Different (p)list/section lengths for (p)list/sections starting "
            "at %d/%d\n",
            t3_config_get_line(a), t3_config_get_line(a));
    break;
  }
